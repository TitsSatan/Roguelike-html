...
function drawActors() {
	for (var a in actorList) {
		if (actorList[a] != null && actorList[a].hp > 0) 
			asciidisplay[actorList[a].y][actorList[a].x].content = a == 0 ? '' + player.hp : 'W';
	}
}
	...
	if (actorMap[newKey] != null) {
		...
		if (victim.hp == 0) {
			...
			if(victim!=player) {
				...
				if (livingEnemies == 0) {
					// victory message
					var victory = game.add.text(game.world.centerX, game.world.centerY, 'Неплохо', { fill : '#2e2', align: "center" } );
...
function aiAct(actor) {
	...
	if (player.hp < 1) {
		// game over message
		var gameOver = game.add.text(game.world.centerX, game.world.centerY, 'АХАХХАХХАХ', { fill : '#e22', align: "center" } );
